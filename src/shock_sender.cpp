...
ShockLogger* shockLogger = nullptr;
...
class PiShockPayload {
	...
	public:
		...
		void set_publish_commands(const std::vector<PublishCommandT>& value) {
			this->PublishCommands = value;
		}
...
const std::string getWarningMessage(int maxShockValue) {
	...
	ss << "Your max shock value is set at " << maxShockValue << "\n"
		 ...
		 << "If this is intentional, make sure to have someone closeby to help you if something goes "
				"wrong, along with a first aid kid\n"
		 << "Do you accept the health risks that come with this value, and wish to proceed anyway? If "
				"you do not, the game will use a max value of 100 instead";
	return ss.str();
}
...
int showErrorMessage(const std::string& msg, const std::string& type) {
	return MessageBoxA(
		NULL,
		msg.c_str(),
		type == "warn" ? "DANGER" : "ERROR",
		type == "warn" ? MB_ICONWARNING | MB_YESNO : MB_ICONERROR
	);
}
...
net::awaitable<boost::json::value> sendGetRequest(std::string path, std::string subPath) {
	...
	shockLogger->info("Sending GET to " + path + subPath);

	tcp::resolver resolver(ex);
	beast::tcp_stream stream(ex);

	auto results = co_await resolver.async_resolve(path, "80", net::use_awaitable);

	co_await stream.async_connect(results, net::use_awaitable);

	http::request<http::string_body> req{http::verb::get, subPath, 11};
	...
	shockLogger->info("HTTP response received");
	auto body = res.body();
	shockLogger->debug(body);

	co_return boost::json::parse(body);
}

net::awaitable<boost::json::value> sendGetRequest(
	std::string path,
	std::string subPath,
...
net::awaitable<void> getUserId() {
	auto result = co_await sendGetRequest(
		authHost,
...
net::awaitable<void> getShockerIds() {
	auto result = co_await sendGetRequest(
		psHost,
		"/PiShock/GetUserDevices",
		{{"UserId", std::to_string(userId)}, {"Token", apiKey}, {"api", "true"}}
	);
...
LogLevel getLogLevel(const std::string& pathToFile) {
	std::ifstream configFile(pathToFile);
	if (configFile.bad()) {
		configFile.close();
	...
	while (std::getline(configFile, line)) {
		line = removeComments(line);
		if (boost::starts_with(line, "log_level"))
			break;
	}
	configFile.close();
	if (line.empty()) {
	...
	std::tuple<std::string, std::string> keyAndVal = getKeyAndVal(line);
	std::string& key = get<0>(keyAndVal);
	if (!boost::equals(key, "log_level"))
		return LogLevel::DEBUG;
	std::string& value = get<1>(keyAndVal);
	boost::trim(value);
	if (boost::istarts_with(value, "err"))
		return LogLevel::ERR;
	if (boost::istarts_with(value, "warn"))
		return LogLevel::WARN;
	if (boost::istarts_with(value, "info"))
		return LogLevel::INFO;
	return LogLevel::DEBUG;
}

net::awaitable<void> startWebsocket() {
	shockLogger->debug("This is the start websocket function");
	...
	co_await net::async_connect(ws.next_layer(), results, net::use_awaitable);
	co_await ws.async_handshake(wsHost, target, net::use_awaitable);
	isRunning = true;
	shockLogger->debug("Websocket is running");
	beast::flat_buffer buffer;
	while (ws.is_open()) {
		std::size_t n = co_await ws.async_read(buffer, net::use_awaitable);
		std::string msg{static_cast<char const*>(buffer.data().data()), n};
		buffer.consume(n);
	}
}
	...
	net::co_spawn(ioc, initializeClient(), [&](std::exception_ptr e) {
		if (e) {
			try {
				...
				if (shockLogger != nullptr)
					shockLogger->err(ex.what());
			}
		}
	});
	ioc.run();
	if (shockLogger != nullptr)
		shockLogger->info("Connection loop broken");
}
...
void checkMaxShockOk(std::string pathToFile) {
	...
	std::string line;
	while (std::getline(configFile, line)) {
		if (boost::starts_with(line, "max_shock"))
			break;
	}
	configFile.close();
	line = removeComments(line);
...
extern "C" EXPORT void startShockSender(char* filePath) {
	...
	try {
		...
		shockLogger = new ShockLogger(logLevel);
		shockLogger->debug("File path " + pathToFile);
		std::jthread checkThread(checkMaxShockOk, pathToFile);
		checkThread.detach();
		getUsernameAndApiKey(pathToFile);
		warnAboutEmptyValues();
		shockLogger->debug("Username " + username);
		shockLogger->debug("API key " + apiKey.empty() ? "missing" : "ok");
		std::jthread ioThread(startLibrary);
	} catch (std::exception& e) {
		shockLogger->err(e.what());
	}
}
...
bool getIsContinuous() {
	...
	std::string line;
	while (std::getline(configFile, line)) {
		if (boost::istarts_with(line, "continuous"))
			break;
	}
	line = removeComments(line);
	// default to false
...
const std::string getMode() {
	...
	std::string line;
	while (std::getline(configFile, line)) {
		if (boost::istarts_with(line, "mode"))
			break;
	}
	line = removeComments(line);
	// default to vibrate
...
extern "C" EXPORT int sendShock(int amount, bool useQuarters) {
	...
	bool isContinuous = getIsContinuous();
	MaxShockAndIntensityPerQuarter maxShockAndIntensityPerQuarter =
		getMaxShockAndIntensityPerQuarter();
	...
	bool okayToSend = maxShockAndIntensityPerQuarter.maxShockIntensity <= 100 ||
		(userAuthorizedDangerousValue && userWarnedAboutMaxShock);
	if (!okayToSend) {
		shockLogger->warn("Max value too high, aborting.");
		return 1;
	}
	...
	shockLogger->info(
		"Preparing " + std::to_string(commands.size()) + " shocks with an intensity of " +
		std::to_string(intensity) +
		(isContinuous ? " continuously"
									: (" and a duration of " +
										 std::to_string(maxShockAndIntensityPerQuarter.durationMilliseconds) + "ms"))
	);
